...
const generateUsersList = (usersArray, template, parent) => {
  if (usersArray.length <= 0) {
    parent.textContent = 'Пока пользователей нет, добавьте нового пользователя.'
    return
  }
...
const createGameCategoriesForm = (gameId, categoriesState, currentState) => {
  ...
  //расставляем checked
  let targetCategories = categoriesState.filter(item => currentState.categories.includes(item._id))
  const checkboxes = [...document.querySelectorAll(`#game-${gameId} .game-categories-form input`)]
...
const removeGameCategoriesForm = gameId => {
  const categoriesForm = document.querySelector(`#game-${gameId} .game-categories-form`)
