...
const postNewGame = async form => {
  ...
  try {
    const response = await postData('/api/game', object)
...
const postNewCategory = async form => {
  const object = prepareFormData(form)
  try {
    const response = await postData('/api/category', object)
    if (response instanceof Error) {
      throw new Error(response.message)
    }
    return response
...
const postNewUser = async form => {
  const object = prepareFormData(form)
  try {
    const response = await postData('/api/auth/registration', object)
    if (response instanceof Error) {
      throw new Error(response.message)
    }
    return response
...
const addDeleteUsersListeners = async () => {
  ...
  deleteButtons.forEach(button => {
    button.addEventListener('click', async event => {
      event.preventDefault()
      const id = event.currentTarget.dataset.id
      const approved = confirm('Вы уверены, что хотите удалить этого пользователя?')
      if (approved) {
...
const putGame = async id => {
  try {
    const response = await putData(`/api/game/${id}`, currentState)
    if (response instanceof Error) {
      throw new Error(response.message)
    }
    return response
...
const putCategory = async id => {
  try {
    const response = await putData(`/api/category/${id}`, currentCategoryState)
    if (response instanceof Error) {
      throw new Error(response.message)
    }
    return response
